...
for (i in 1:4) {
  ...
  chosenMen     <- scales[[2]][dass[[1]]$gender == 1, i]
  chosenWomen   <- scales[[2]][dass[[1]]$gender == 2, i]
  p             <- t.test(chosenMen, chosenWomen,
                          var.equal = FALSE)$p.value
...
colnames(df) <- c("Working-age cohort.Men (n = 640)",
                  "Working-age cohort.Women (n = 2566)",
                  "Working-age cohort.p",
                  "Working-age cohort.Total (n = 3206)",
...
niceDf       <- rempsyc::nice_table(df,
                           title = c("Table 6.",
                                     "Mean levels of depression, anxiety, stress and general distress among the men and women of the two cohorts of the Helsinki Health Study in 2017."),
